...
description: >-
  ...
  K8s version upgrades -- run this before using prow-aks-jobs to make changes.
---
...
- Before updating the AKS K8s version (use `prow-aks-jobs` to make changes)
...
**Always update the main branch to the newest GA version.** If the configured version on main is not the newest GA version, proceed to update it using the `prow-aks-jobs` skill to change `MAPT_KUBERNETES_VERSION` in the main CI config file.

For release branches (e.g., release-1.9, release-1.8), **ask the user** whether they should also be updated before making changes.

## Related Skills

- **`prow-aks-jobs`**: Update the AKS K8s version and list test entries
