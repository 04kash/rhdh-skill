...
description: >-
  ...
  planning EKS K8s version upgrades -- run this before using prow-eks-jobs to
  make changes.
---
...
- Before updating the EKS K8s version (use `prow-eks-jobs` to make changes)
...
**Always update the main branch to the newest Standard version.** If the configured version on main is not the newest Standard version, proceed to update it using the `prow-eks-jobs` skill to change `MAPT_KUBERNETES_VERSION` in the main CI config file.

For release branches (e.g., release-1.9, release-1.8), **ask the user** whether they should also be updated before making changes.

## Related Skills

- **`prow-eks-jobs`**: Update the EKS K8s version and list test entries
